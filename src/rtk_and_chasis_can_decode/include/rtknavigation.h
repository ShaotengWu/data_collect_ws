...
void rtknavigation::publishrtk()
{
    ...
    for (auto frame = canstream_->vdata_.cbegin(); frame != canstream_->vdata_.cend(); frame++)
    {
        ...
        if (frame->uID == 0x327)
        {
            rtk.velE = CAN_decode(*frame, 8, 16, 1e-2, 0, 1);
            rtk.velN = CAN_decode(*frame, 24, 16, 1e-2, 0, 1);
            rtk.velU = CAN_decode(*frame, 40, 16, 1e-2, 0, 1);
            rtk.velocity = CAN_decode(*frame, 56, 16, 1e-2, 0, 1);
        }

        if (frame->uID == 0x329)
        {
            rtk.AccelX = CAN_decode(*frame, 20, 20, 1e-4, 0, 1);
            rtk.AccelY = CAN_decode(*frame, 32, 20, 1e-4, 0, 1);
            rtk.AccelZ = CAN_decode(*frame, 60, 20, 1e-4, 0, 1);
        }
        if (frame->uID == 0x32C)
        {
            rtk.angRateX = CAN_decode(*frame, 20, 20, 1e-2, 0, 1);
            rtk.angRateY = CAN_decode(*frame, 32, 20, 1e-2, 0, 1);
            rtk.angRateZ = CAN_decode(*frame, 60, 20, 1e-2, 0, 1);
        }
